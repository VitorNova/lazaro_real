...
from app.services.asaas import AsaasService, create_asaas_service
...
async def sync_payments_to_cache(
    ...
    _log(f"Cache atualizado: {synced_count} payments sincronizados, {error_count} erros")
...
async def save_cobranca_enviada(
    ...
    Usa UPSERT para atualizar registro existente (criado por claim_notification) com dados completos.
    """
    supabase = get_supabase_service()
    try:
        ...
        # UPSERT: atualiza registro existente (criado por claim_notification) com dados completos
...
async def process_agent_billing(agent: Dict[str, Any]) -> Dict[str, int]:
    ...
    async def send_notification(
        ...
        try:
            ...
            # ================================================================
            # TRANSFERIR PARA FILA DE COBRANÇA (SE CONFIGURADO)
            # Após enviar a mensagem, transfere o lead para o departamento de cobrança
            # para que atendentes humanos possam acompanhar
            # ================================================================
            try:
                handoff_triggers = agent.get("handoff_triggers") or {}
                dispatch_departments = handoff_triggers.get("dispatch_departments") or {}
                billing_dept = dispatch_departments.get("billing")

                if billing_dept and handoff_triggers.get("enabled"):
                    billing_queue_id = billing_dept.get("queueId")
                    billing_user_id = billing_dept.get("userId")

                    if billing_queue_id:
                        _log(f"Transferindo lead {mask_phone(phone)} para fila de cobrança (queue={billing_queue_id}, user={billing_user_id})")

                        # Criar cliente Leadbox
                        lb_api_url = handoff_triggers.get("api_url")
                        lb_api_uuid = handoff_triggers.get("api_uuid")
                        lb_api_token = handoff_triggers.get("api_token")

                        if lb_api_url and lb_api_uuid and lb_api_token:
                            leadbox_client = LeadboxService(
                                base_url=lb_api_url,
                                api_uuid=lb_api_uuid,
                                api_key=lb_api_token
                            )

                            transfer_result = await leadbox_client.transfer_to_department(
                                phone=phone,
                                queue_id=billing_queue_id,
                                user_id=billing_user_id,
                                notes=f"Cobrança enviada automaticamente: {notification_type}",
                                mensagem=f"Lead transferido automaticamente após envio de cobrança (payment_id: {payment['id']})"
                            )

                            if transfer_result.get("sucesso"):
                                _log(f"Lead transferido com sucesso para fila de cobrança: ticket_id={transfer_result.get('ticket_id')}")
                            else:
                                _log_warn(f"Falha ao transferir lead para fila de cobrança: {transfer_result.get('mensagem')}")
                        else:
                            _log_warn("Leadbox configurado mas sem credenciais (api_url, api_uuid, api_token)")
                    else:
                        _log_warn("Departamento billing configurado mas sem queueId")

            except Exception as e:
                # Transferência é best-effort - não deve falhar o envio da cobrança
                _log_warn(f"Erro ao transferir lead para fila de cobrança (não-crítico): {e}")
    ...
    for days_ahead in reminder_days:
        ...
        if source == "api":
            ...
            import asyncio
            asyncio.create_task(sync_payments_to_cache(agent["id"], payments))
