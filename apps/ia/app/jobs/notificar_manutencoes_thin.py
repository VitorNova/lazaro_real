"""
Maintenance Notifier Job (Thin) - Entry point para notificacao de manutencao preventiva.

Logica de negocio delegada para: domain/maintenance/services/notification_service.py
...
async def run_maintenance_notifier_job() -> Dict[str, Any]:
    ...
    try:
        ...
        if not agent:
            logger.warning(f"[MAINTENANCE JOB] Agente nao encontrado")
...
async def _force_run_maintenance_notifier() -> Dict[str, Any]:
    """Versao forcada - ignora verificacoes. APENAS PARA DEBUG."""
    global _is_running
